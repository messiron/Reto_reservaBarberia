...
import com.mycompany.barberia.Reserva;
...
public class ManipularReserva {
    ...
    public void createRegistro(Reserva reservaAdd) {
        ...
        try {
            ...
            file = new File("Reserva.txt");
            if(file.exists()){
             FileOutputStream archivo2 = new FileOutputStream("Reserva.txt", true);
            ...
            FileOutputStream archivo = new FileOutputStream("Reserva.txt", true);
    ...
    HashMap<Integer,Reserva> hashReserva= new HashMap<>();
    ...
    while(true){
        try{
            ...
            hashReserva.put(reserva.getId(), reserva);
        }catch(Exception e){
        break;
        }
    }
      filein.close();
      in.close();
      if(hashReserva.containsKey(id));
      hashReserva.remove(id);
      updateFile(hashReserva, nombreArchivo);
    ...
    public void updateFile(HashMap<Integer, Reserva> hashUpdate, String nombreArchivo){
              ...
              try {
                ...
                for(Reserva reserva  : hashUpdate.values()){
                    ojectOutputStream.writeObject(reserva);
                }
